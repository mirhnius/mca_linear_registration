...
PATTERN = (
    pathlib.Path("") / "sub-*" / BASELINE_SESSION / ANATOMICAL / f"sub-*_{BASELINE_SESSION}_{ACCUSITION}_{RUN}_{MOSUF}"
)
...
def find_scans(input_dir: pathlib.Path, pattern=PATTERN, sub_dirs: List[str] = None) -> List[pathlib.Path]:
    ...
    if sub_dirs is None:
        ...
        scan_paths = []
        pattern = (
            pathlib.Path("") / BASELINE_SESSION / ANATOMICAL / f"sub-*_{BASELINE_SESSION}_{ACCUSITION}_{RUN}_{MOSUF}"
        )
...
def create_bet_invocation(subject: Dict, output_dir: pathlib.Path, f=0.5) -> Dict:
    ...
    invocations = {"infile": str(in_file), "maskfile": str(out_file), "fractional_intensity": f}  # output
...
def create_robustfov_invocation(subject: Dict, output_dir: pathlib.Path):
    ...
    invocations = {"in_file": str(in_file), "out_roi": str(out_file)}
...
def make_robustfov_invocations(
    subjects_map: Dict,
...
def make_bet_invocations(
    subjects_map: Dict, output_dir: pathlib.Path, invocation_dir: pathlib.Path, f=0.5, dry_run: bool = False
):
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        fromfile_prefix_chars="@", description="create invocation for MCA and Original data"
    )
    parser.add_argument("--input_dir", type=str, help="path to the input directory")
    parser.add_argument("--output_dir", type=str, help="path to the output directory")
    parser.add_argument(
        "--invocation_dir", type=str, default=pathlib.Path().cwd() / "invocations", help="path to invocation directory"
    )
...
if __name__ == "__main__":
    ...
    subjects_map_after_preprocess = make_robustfov_invocations(
        subjects_map, robustfov_output_dir, robustfov_invocation_dir, flag=True, dry_run=args.dry_run
    )
    ...
    make_flirt_invocations(
        subjects_map_after_preprocess, output_dir, flirt_invocation_dir, n_mca=args.n_mca, dry_run=args.dry_run
    )
