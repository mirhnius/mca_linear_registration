...
class FLIRT_MCA_preprocessing(FLIRT_preprocessing):
    def __init__(
        ...
        """
        Initializes the FLIRT preprocessing class for MCA (Monte Carlo Arithmetic)
        with subject maps, output directory, invocation directory,
        reference image, degrees of freedom, and the number of MCA iterations.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="create invocation for MCA and Original data",
    )
    ...
    parser.add_argument(
        "--invocation_dir",
        type=str,
        default=pathlib.Path().cwd() / "invocations",
        help="path to invocation directory",
    )
...
if __name__ == "__main__":
    ...
    ROBUSTFOV_preprocessing(subjects_map, robustfov_output_dir, robustfov_invocation_dir).create_invocations(
        dry_run=args.dry_run
    )

    BET_preprocessing(subjects_map_after_preprocess, bet_output_dir, bet_invocation_dir).create_invocations(
        dry_run=args.dry_run
    )

    FLIRT_IEEE_preprocessing(subjects_map_after_preprocess, output_dir, flirt_invocation_dir).create_invocations(
        dry_run=args.dry_run
    )

    FLIRT_MCA_preprocessing(
        subjects_map_after_preprocess,
        output_dir,
        flirt_invocation_dir,
        n_mca=args.n_mca,
